...
class OptimizedAgentService:
    ...
    def __init__(self):
        self._llm_scoring_enabled = True  # 可以配置开关
        self._min_score_for_llm = 0.3  # 低于此分数才启用 LLM 重评分
    ...
    def _ai_score_files(self, query: str, related_files: List[dict]) -> List[dict]:
        ...
        if not self._llm_scoring_enabled:
            return related_files

        if len(related_files) <= 5:
            return related_files

        if not self._needs_llm_rerank(query, related_files):
            return related_files

        file_descriptions = []
        for i, file in enumerate(related_files[:10]):
            desc = f"""文件 {i+1}:
...
- 不要有任何其他文字"""
    ...
    def _needs_llm_rerank(self, query: str, related_files: List[dict]) -> bool:
        """判断是否真的需要 LLM 重评分。"""
        max_score = max((f.get("score", 0) for f in related_files), default=0)
        if max_score >= self._min_score_for_llm:
            return False
        return True
