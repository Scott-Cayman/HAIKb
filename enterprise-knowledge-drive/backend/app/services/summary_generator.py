...
class SummaryGeneratorService:
    ...
    def generate_summary(self, file: File, parsed: Dict[str, object]) -> Dict[str, object]:
        ...
        logger.info(f"[SummaryGenerator] 开始处理文件: id={file.id}, name={file.original_name}")
        logger.info(f"[SummaryGenerator] parsed 数据: {list(parsed.keys())}")
        logger.info(f"[SummaryGenerator] image_path: {image_path}")
        ...
        if image_path and llm_service.is_vision_configured():
            logger.info(f"[SummaryGenerator] 尝试使用视觉模型")
            try:
                markdown = self._generate_with_vision_llm(file, image_path, parse_pages, parse_confidence)
        ...
        # 如果都没成功，用规则兜底
        if not markdown:
            logger.info(f"[SummaryGenerator] 使用规则兜底")
    ...
    def _generate_with_vision_llm(self, file: File, image_path: str, parse_pages: int, parse_confidence: str) -> str:
        """使用视觉模型直接识别图片并生成总结。"""
        import logging
        logger = logging.getLogger(__name__)
        logger.info(f"[VisionLLM] 开始生成，image_path={image_path}")
        ...
        user_prompt = f"""请分析这张图片，这是文件名：{file.original_name}。
请按照上方的格式要求生成一份结构化的 Markdown 总结。
注意：请完整保持 Markdown 格式，包括 # AI_DOCUMENT_SUMMARY 标题和所有小节。"""
        
        result = llm_service.chat_with_image(
        ...
        logger.info(f"[VisionLLM] 完成，结果长度: {len(result)}")
        return result
...
"""
    ...
    def extract_structured_fields(self, markdown: str) -> Dict[str, object]:
        ...
        # 如果结构化标签解析失败，尝试从简单标签列表中推断
        if client_type == "未识别" and project_type == "未识别" and tags_section:
            keywords = [line.strip("- ").strip() for line in tags_section.splitlines() if line.strip().startswith("-")]
            
            if any(k in keywords for k in ["政府", "国企", "央企", "事业单位"]):
                client_type = "政府客户"
            elif any(k in keywords for k in ["互联网", "科技", "电商", "金融"]):
                client_type = "企业客户"
            
            if any(k in keywords for k in ["招标", "投标", "竞标"]):
                project_type = "招投标"
            elif any(k in keywords for k in ["会议", "汇报", "培训"]):
                project_type = "会议汇报"
            
            if any(k in keywords for k in ["方案", "设计", "技术"]):
                document_type = "项目方案"
    ...
    def _needs_structured_fallback(self, fields: Dict[str, object]) -> bool:
        # 如果 one_line 和 two_sentence 都有内容，就不回退！
        text_fields = [
            str(fields.get("one_line_judgement") or "").strip(),
            str(fields.get("two_sentence_intro") or "").strip(),
        ]
        
        # 如果两个文本字段都有内容，就不回退！
        text_valid = all(value and value != "未识别" for value in text_fields)
        if text_valid:
            return False
        
        # 否则才检查标签
        label_fields = [
            str(fields.get("client_type") or "").strip(),
            str(fields.get("project_type") or "").strip(),
            str(fields.get("document_type") or "").strip(),
        ]
        tag_fields = [
            str(fields.get("industry_tags") or "").strip(),
            str(fields.get("region_tags") or "").strip(),
            str(fields.get("keyword_tags") or "").strip(),
        ]
        text_missing = any(not value for value in text_fields)
        labels_missing = all(value in {"", "未识别"} for value in label_fields)
        tags_missing = all(value in {"", "[]", '["未识别"]'} for value in tag_fields)
        return text_missing or (labels_missing and tags_missing)

    def _is_better_structured_payload(self, candidate: Dict[str, object], current: Dict[str, object]) -> bool:
        def score(payload: Dict[str, object]) -> int:
            total = 0
            for key in ("one_line_judgement", "two_sentence_intro", "client_type", "project_type", "document_type"):
                value = str(payload.get(key) or "").strip()
                if value and value != "未识别":
                    total += 1
            for key in ("industry_tags", "region_tags", "keyword_tags"):
                value = str(payload.get(key) or "").strip()
                if value not in {"", "[]", '["未识别"]'}:
                    total += 1
            return total
