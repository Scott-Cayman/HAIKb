import { Box, BookOpen, GraduationCap, Sparkles } from 'lucide-react';
import { gradientToCss, radialGlowToCss, type GradientColors } from '../../config/homeAppearance';
...
const iconMap = [BookOpen, GraduationCap, Box, Sparkles];

const FolderCoverArt = ({ variant, imageUrl, iconGradient, iconColor, glowColor }: FolderCoverArtProps) => {
  if (imageUrl) {
    return (
      <img
        src={imageUrl}
  ...
  const Icon = iconMap[variant % iconMap.length];
  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      ...
      <div
        className="absolute bottom-7 right-8 flex h-16 w-16 items-center justify-center rounded-[1.4rem] shadow-[0_18px_28px_rgba(99,102,241,0.18)]"
        style={{ backgroundImage: gradientToCss(iconGradient) }}
      >
